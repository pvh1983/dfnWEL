...
void ReadBoundaryNodes()
{
  ...
  int zonenumb_in=0, zonenumb_out=0, zonenumb_inj=0;
  ...
  inputfile = Control_Data("inj_well_zone:",14 );
  zonenumb_inj=inputfile.flag;  

  char  line[10]={0};
  int  nzone_out, i,  fn, nn, res, nf, flag1=0, flag2=0, flag3=0;
  if (fscanf(fpc, "%s \n", line)!=1)
    i=i;
  do
    {
      ...
      // scan allboundaries.zone to find id of in/out boundary and read #ofnodes. 
      if  (fn==zonenumb_in) {    //in-flow-boundary:
		  flag1=1;
		  ...
		  for (i=0; i<nzone_in; i++)     //find the id of nodes in inflow boundary
			{
	   ...
	   }   // end of if  (fn==zonenumb_in)
      else if  (fn==zonenumb_inj) {    //hpham: injection well:
		  flag3=1;
//		  printf("zonenumb_inj = %d \n", zonenumb_inj); 
		  if (fscanf(fpc, " %s ", line)!=1)
			flag3=flag3;

		  if (fscanf(fpc, " %d", & nnode_iwel)!=1)
			nnode_iwel=nnode_iwel;     // The number of nodes in the injection well zone.
//		  printf("nnode_iwel = %d \n", nnode_iwel); 

		  /** memory allocation for nodezone_in nodes ******/
		  nodeinj_wel=(unsigned int*) malloc (nnode_iwel*sizeof(unsigned int));

		  for (i=0; i<nnode_iwel; i++)     //find the id of nodes in inflow boundary
			{
			  if (fscanf(fpc," %d ", &nf)!=1)
				i=i;
			  nodeinj_wel[i]=nf;
			  if(node[nodeinj_wel[i]-1].typeN<300)
				node[nodeinj_wel[i]-1].typeN=node[nodeinj_wel[i]-1].typeN+300;
			}
	   }   // end of if  (fn==zonenumb_in)


      else {
      // read out-flow-boundary 
	  if (fn==zonenumb_out)
  ...
  printf("\n [RGI.c] Number of nodes in the flow-in zone: %d \n", nzone_in);
  printf(" [RGI.c] Number of nodes in the flow-out  zone: %d \n", nzone_out);
  printf(" [RGI.c] Number of nodes in the injection well zone: %d \n", nnode_iwel);     //hpham
  ...
  /*** Calculate the sum of mass fluxes on the inflow boundary and on the outflow boundary
       hpham modified to check mass balance ***/
  double density1=0.0;
  ...
  printf("\n [RGI.c] Total in-flow volumetric flux = %12.5e [m^3/s]\n", sum_in);
  printf(" [RGI.c] Total out-flow volumetric flux = %12.5e [m^3/s]\n", sum_out);     //hpham
  printf(" [RGI.c] where 1: top;  2: bottom;  3: left_w;  4: front_s;  5: right_e and  6: back_n \n");  //hpham added this line
  printf("\n");
  /***************************/
...
void ReadBoundaryNodes_hpham()
{
  struct inpfile inputfile;

  /****Functions reads the node's numbers assigned in BC from boundary.zone file *************/
  inputfile = Control_File("boundary:",9 );  
  FILE *fpc = OpenFile (inputfile.filename,"r");

  int zone_id, nnodes_in_azone;
  unsigned int *node_id_of_azone;     // hpham added this, an array that stores all node_id in a zone. 
  char  line[10]={0};
  int i, j, res, fn, nf, flag1=0, nfrac_in_zone;
  double sum_flux,sum_flux_inside, wbalance=0;
  static FILE *fid_p11;   // hpham
  static FILE *fid_flx;   // hpham
  
  // Open a file to write p11 at wells Length of Fracture Intersects per 
  // Unit Length of Scanline
//  sprintf(filename,"p11.out");     // hpham
  fid_p11 = OpenFile("p11.out","w");      // hpham
  fid_flx = OpenFile("flux.out","w");	    // hpham
  fprintf(fid_p11,"p11, sum_flux \n");
  if (fscanf(fpc, "%s \n", line)!=1)
    i=i;     // Read and skip the first line of allboundaries.zone (word: zone)
  
//  do     // loop until seeing the word "stop"
	for (zone_id=1; zone_id < 10; zone_id++)
    {
      fprintf(fid_flx,"zone_id= %d \n",zone_id);
//      fprintf(fid_flx,"i, node_id, flux, pressure, volume, x, y, z \n");
      fprintf(fid_flx,"      i,    node_id, numneighb,         flux,     pressure,       volume,          x,          y,          z \n");
//      printf("zone_id= %d. [Columns are: i, node_id, flux, x, y, z ]\n",zone_id);

      fn=0;
      if (fscanf(fpc," %d  %s ", &fn, line)!=zone_id)
		    fn=fn;    // got id of a zone e.g. fn=000001

	// scan allboundaries.zone to find id of in/out boundary and read #ofnodes. 
		  if (fscanf(fpc, " %s ", line)!=1)
			  flag1=1;  //Read and skip the word "nnum". flag1 marks this zone exist!

		  if (fscanf(fpc, " %d", &nnodes_in_azone)!=1)    // Scan and store all node_id in this zone
			  nnodes_in_azone=nnodes_in_azone;    // number of nodes in each zone
//		  printf("[RGI.c] The number of nodes in zone_id %d is %d. \n", zone_id, nnodes_in_azone); 

			/** memory allocation for nodezone_in nodes ******/
		  node_id_of_azone=(unsigned int*) malloc (nnodes_in_azone*sizeof(unsigned int));

		  for (i=0; i<nnodes_in_azone; i++)     //find the id of nodes in inflow boundary
			{
			  if (fscanf(fpc," %d ", &nf)!=1)
				i=i;
			  node_id_of_azone[i]=nf;
			  if(node[node_id_of_azone[i]-1].typeN<300)
				  node[node_id_of_azone[i]-1].typeN=node[node_id_of_azone[i]-1].typeN+300;
			}

			// Calculate flux for each zone 
			sum_flux=0;
			nfrac_in_zone=1;
			for (i=0; i<nnodes_in_azone; i++)
			  {
//				printf("%d neighboring nodes at the current node. \n", node[node_id_of_azone[i]-1].numneighb);



          sum_flux_inside=0;
			    for (j=0; j<node[node_id_of_azone[i]-1].numneighb; j++)
				  {
				    sum_flux_inside=sum_flux_inside+node[node_id_of_azone[i]-1].flux[j];    //sum_all_neightbor_cells
				  }
        sum_flux=sum_flux+sum_flux_inside; //
        fprintf(fid_flx,"%7d, %10d, %9d, %12.5e, %12.5e, %12.5e, %10.6f, %10.6f, %10.6f \n",i,node_id_of_azone[i], node[node_id_of_azone[i]-1].numneighb, sum_flux_inside, node[node_id_of_azone[i]-1].pressure, node[node_id_of_azone[i]-1].pvolume, node[node_id_of_azone[i]-1].coord[0], node[node_id_of_azone[i]-1].coord[1], node[node_id_of_azone[i]-1].coord[2]); 
        //printf("%7d, %8d, %12.5e, %10.6f, %10.6f, %10.6f \n",i,node_id_of_azone[i],sum_flux_inside,node[node_id_of_azone[i]-1].coord[0], node[node_id_of_azone[i]-1].coord[1], node[node_id_of_azone[i]-1].coord[2]); 

				if ((i>0)&&(node[node_id_of_azone[i]-1].fracture[0]!=node[node_id_of_azone[i-1]-1].fracture[0])) 
				  nfrac_in_zone++; 
				}    // end of loop for nodes
		  printf("[RGI.c] Zone_id %d has %d nodes and %d fractures. Total flux = %12.5e [m^3/s]. \n",zone_id, nnodes_in_azone, nfrac_in_zone, sum_flux); 

      fprintf(fid_p11,"%d %12.5e \n",nfrac_in_zone, sum_flux);
      
      fprintf(fid_flx,"Notes: %d nodes and %d fractures in zone_id %d. Total flux = %12.5e \n",nnodes_in_azone, nfrac_in_zone, zone_id,sum_flux);
      fprintf(fid_flx," \n");
      
//      printf("Notes: %d nodes and %d fractures in zone_id %d. Total flux = %12.5e \n",nnodes_in_azone, nfrac_in_zone, zone_id,sum_flux);
//      printf(" \n");


			if ( (zone_id != 7) && (zone_id != 8) )   // Not for well 2 and well 3
				wbalance=wbalance+sum_flux;
			
//			printf("[RGI.c] Zone_id %3d has sum_fluxes is %12.5e [m^3/s].  \n",zone_id, wbalance);  //hpham added this line 
//	zone_id++;
      free(node_id_of_azone);  //hpham
    }
//  while ((res=strncmp(line,"stop", 4)));
  ...
  fclose(fid_p11);
  fclose(fid_flx);
  printf("[Notes: 1:top; 2:bot; 3:left_w; 4:front_s; 5:right_e; 6:back_n; 7:inj_well; 8:obs_well; 9:pmp_well] \n");  //hpham added this line
  printf("\n Summation of all fluxes (water balance error) = %12.5e [m^3/s].  \n", wbalance);  //hpham added this line
