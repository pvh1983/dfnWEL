...
def get_wel_loc_using_lagrit(wname,x,y,top,bot,r):
    l1='''surface/acyl1/free/cylinder/%f,%f,%f/%f,%f,%f/%f
        region/r1/lt/acyl1
        pset/%s/region/r1
        pset/%s/zone/pointset_%s/ascii''' % (x,y,top,x,y,bot,r,wname,wname,wname)
    return l1
...
'''
    ...
    print("Path to ifile_well:%s", ( p ))
    df_wel = pd.read_csv(os.environ['ifile_well'])
    wname=df_wel.name

    
    # cells in the well zone    
    lagrit_input_21 = ''
    for i,s in enumerate(wname):
        
        x,y = df_wel.x[i], df_wel.y[i]
        top,bot = df_wel.top[i], df_wel.bot[i]
        rw,rs = df_wel.rw[i], df_wel.rs[i]
        script_wel_i = get_wel_loc_using_lagrit(s,x,y,top-0.01,bot+0.01,rw)
        lagrit_input_21 = lagrit_input_21 + '\n' + script_wel_i
    
    
    # cells in the skin zone
    print(wname)
    lagrit_input_22 = ''
    for i,s in enumerate(wname):
        name_skin_zone=s + '_skin'
        print(df_wel) 
        x,y = df_wel.x[i], df_wel.y[i]
        top,bot = df_wel.top[i], df_wel.bot[i]
        rw,rs = df_wel.rw[i], df_wel.rs[i]
        script_wel_i = get_wel_loc_using_lagrit(name_skin_zone,x,y,top+0.01,bot-0.01,rs+0.001)
        lagrit_input_22 = lagrit_input_22 + '\n' + script_wel_i        

    lagrit_input_3 = '''
finish
'''
    lagrit_input = lagrit_input_1 + lagrit_input_21 + lagrit_input_22 + lagrit_input_3
    ...
    files = list(wname) + list(wname+'_skin')
    last_well = files[-1]
    files.remove(last_well)
    # Write all wells but not the last
    for f in files:
    ...
    # Write last well to file first
    ofile = 'pointset_' + last_well + '.zone'
    fzone=open(ofile,"rb")
    lines=fzone.readlines()
    lines=lines[1:]
    fzone.close() 
