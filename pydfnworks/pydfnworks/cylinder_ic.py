...
# Last update: 03/22/2018 new, read vtk as input. 
...
def ic(filename,n_part,cyl_r,cyl_center_top,cyl_center_bottom):

	#filename = "full_mesh.vtk"
	main_elapsed0 = time() - main_time
	timing0 = 'Total time required for loading full_mesh.vtk: %0.2f Minutes'%(main_elapsed0/60.0)
	print timing0

	#n_part = 10000
	#cyl_r = 0.3
	#cyl_center_top=(0.0,65.72,50.00)
	#cyl_center_bottom=(0.0,65.72,-50.00)
	...
	M = np.vstack((x_node-cyl_center_top[0],y_node-cyl_center_top[1])) #matrix of node locations relative to cylinder center
	r = np.sqrt(np.sum(np.multiply(M,M),axis=0)) #distance between node and cylinder center
	x_node[r>cyl_r] = np.nan #mark nodes that are outside of cylinder
	x_node[z_node>cyl_center_top[2]] = np.nan #mark nodes that are outside of cylinder
	x_node[z_node<cyl_center_bottom[2]] = np.nan #mark nodes that are outside of cylinder
	...
	print timing2
	print 'End of file cylinder_ic.py.'
	print ' '
